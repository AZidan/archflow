...
- **Yes** — copy each missing file from `.github/archflow/skills/archflow/` to `.archflow/`. Create subdirectories (`phases/`, `schemas/`, `design-systems/`, `design-systems/examples/`) if needed. NEVER overwrite existing files.
...
```
- **Yes** — read `.github/archflow/commands/design.md` and follow **Step 3 — `pick`** inline.
...
1. **Detect the stack.** Read and follow
   `.github/archflow/skills/archflow/stack-detection.md` — the evidence sources, the
...
1. Check if the tool's MCP is configured:
   ...
   ```
2. If NOT configured:
   - Run `/archflow-setup-mcp [tool]` inline (load `.github/archflow/commands/setup-mcp.md`)
   - If MCP setup requires restart: save progress to `.onboard-progress.yaml`, instruct user to restart Copilot, then run `/archflow-onboard` again
...
- **Pick another** (and whenever nothing was detected) — read
  `.github/archflow/commands/design.md` and follow **Step 3 — `pick`** inline. It filters the
...
**1b. Doc Deep-Dive (Task subagent, (delegate all, then collect results))**
...
**1c. Design Extraction (Task subagent, (delegate all, then collect results))**
...
**1d. Route/API Extraction (Task subagent, (delegate all, then collect results))**
...
If an MCP needs to be configured and requires a Copilot restart:
1. Save all progress to `.onboard-progress.yaml`
2. Tell the user: "Please restart Copilot, then run `/archflow-onboard` to resume."
