---
name: archflow-init
description: "Use ONLY when the user asks for /archflow-init or \"archflow init\". Set up Archflow in a NEW, empty project — creates .archflow/ and starts at Phase 1"
---
...
```
Then load and follow `.github/archflow/commands/onboard.md`.
...
2. **Copy `workflow.md` into the project's `.archflow/`:**
   - Source: `.github/archflow/skills/archflow/workflow.md` (the plugin's install path)
...
- `.github/archflow/skills/archflow/phases/` → `.archflow/phases/`
- `.github/archflow/skills/archflow/schemas/` → `.archflow/schemas/`
- `.github/archflow/skills/archflow/design-systems/` → `.archflow/design-systems/`
- `.github/archflow/skills/archflow/stacks/` → `.archflow/stacks/`
- `.github/archflow/skills/archflow/workflow.md` → `.archflow/workflow.md`
- `.github/archflow/skills/archflow/test-accounts.example.yaml` → `.archflow/`
...
1. **Ask the platform**, then run the picker. Both live in one place: read
   `.github/archflow/commands/design.md` and follow **Step 3 — `pick`** inline. It asks the
...
Write the section below into `AGENTS.md` (every host reads it) and also into `CLAUDE.md` (skip: Claude Code only).
...
Created:
  ...
  CLAUDE.md (skip: Claude Code only)
