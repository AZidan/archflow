---
name: archflow-release
description: "Use ONLY when the user asks for /archflow-release or \"archflow release\". Release pipeline: see status, cut a new release, start building it, or ship it"
---
...
Arguments (`the text the user wrote after the skill name`): `new [name]`, `start [slug]`, `ship`, or empty for status.
...
3. Invoke **feature-planner (Mode B)**: select stubs from `backlog.yaml` and MOVE them into
   ...
   as-is; any still-bare stub is groomed first, following `.github/archflow/commands/groom.md` for AC shape and
