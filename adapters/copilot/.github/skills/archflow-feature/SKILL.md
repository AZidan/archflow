---
name: archflow-feature
description: "Use ONLY when the user asks for /archflow-feature or \"archflow feature\". Add a story (name or description), or pull a backlog story into the active release"
---
...
Argument (`the text the user wrote after the skill name`): optional. Either a feature **name or free-text description** (used to pre-fill the wizard — see Option A), or an **existing story id** (e.g. `S2-11`) to pull into the active release. Empty → interactive wizard from scratch.
...
If `the text the user wrote after the skill name` was given and is not a story id, use it to pre-fill: a short phrase becomes the
...
5. User picks one:
   ...
   - **A backlog stub** → this is a pull-forward: promote/MOVE it into the active release, then Step 3.
     If the stub is not yet groomed (`status: backlog`), run `.github/archflow/commands/groom.md` FIRST to detail it —
