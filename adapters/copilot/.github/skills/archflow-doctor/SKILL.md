...
| `claude` on PATH | **WARN** | Only `/archflow-studio` needs it | Already present if Copilot runs |
...
```bash
python3 ".github/archflow/scripts/validate_archflow.py" .
```
...
```bash
python3 ".github/archflow/scripts/upgrade_archflow.py" . \
  --plugin-root ".github/archflow"
```
...
```bash
python3 ".github/archflow/scripts/upgrade_archflow.py" . \
  --plugin-root ".github/archflow" --apply
```
...
what is installed, the marketplace plugin on Copilot or the copied adapter elsewhere. It reads `~/.cache/archflow-install/latest.json`,
refreshed in the background at most once a day, and is silenced by `update_check: false` in
`project-settings.yaml`. Report the same comparison here when that file exists. The fix on Copilot
...
1. **Detect**, following `.github/archflow/skills/archflow/stack-detection.md`. That file is the
...
On Copilot the plugin's `PreToolUse` hook stops an agent force-pushing to `main` or pushing during
...
```bash
sh ".github/archflow/scripts/archflow-install-git-guard.sh"
```
