...
Configure external tool MCP servers for use with Copilot.
...
## Registry
The curated MCP registry is stored in `.github/archflow/skills/archflow/mcp-registry.yaml`. Read that file to look up tool details. Edit it to add or update MCP servers.
...
Read `.github/archflow/skills/archflow/mcp-registry.yaml` and search for the tool name (case-insensitive). If found, show:
...
> "Configured [Tool] MCP. Restart Copilot or run `/mcp` to verify the connection."
...
When invoked without arguments (`/archflow-setup-mcp`), read `.github/archflow/skills/archflow/mcp-registry.yaml` and show the registry as a table:
...
- Some MCPs require Copilot restart after configuration.
