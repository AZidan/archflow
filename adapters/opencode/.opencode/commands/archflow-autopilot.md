...
`.opencode/archflow/commands/groom.md` inline — the user is still here) / `Drop it from this run`.
...
building on an unknown base. Re-ask (via `the `question` tool`) only questions for stories that were
