...
- **Yes** — copy each missing file from `.opencode/archflow/skills/archflow/` to `.archflow/`. Create subdirectories (`phases/`, `schemas/`, `design-systems/`, `design-systems/examples/`) if needed. NEVER overwrite existing files.
...
```
- **Yes** — read `.opencode/archflow/commands/design.md` and follow **Step 3 — `pick`** inline.
...
1. **Detect the stack.** Read and follow
   `.opencode/archflow/skills/archflow/stack-detection.md` — the evidence sources, the
...
1. Check if the tool's MCP is configured:
   ...
   ```
2. If NOT configured:
   - Run `/archflow-setup-mcp [tool]` inline (load `.opencode/archflow/commands/setup-mcp.md`)
   - If MCP setup requires restart: save progress to `.onboard-progress.yaml`, instruct user to restart OpenCode, then run `/archflow-onboard` again
...
- **Pick another** (and whenever nothing was detected) — read
  `.opencode/archflow/commands/design.md` and follow **Step 3 — `pick`** inline. It filters the
...
**1b. Doc Deep-Dive (Task subagent, (launch all, then collect results))**
...
**1c. Design Extraction (Task subagent, (launch all, then collect results))**
...
**1d. Route/API Extraction (Task subagent, (launch all, then collect results))**
...
If an MCP needs to be configured and requires a OpenCode restart:
1. Save all progress to `.onboard-progress.yaml`
2. Tell the user: "Please restart OpenCode, then run `/archflow-onboard` to resume."
