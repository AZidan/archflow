...
| `claude` on PATH | **WARN** | Only `/archflow-studio` needs it | Already present if OpenCode runs |
...
```bash
python3 ".opencode/archflow/scripts/validate_archflow.py" .
```
...
```bash
python3 ".opencode/archflow/scripts/upgrade_archflow.py" . \
  --plugin-root ".opencode/archflow"
```
...
```bash
python3 ".opencode/archflow/scripts/upgrade_archflow.py" . \
  --plugin-root ".opencode/archflow" --apply
```
...
**Newer release.** On hosts other than OpenCode the session-start hook also reports when a newer
...
OpenCode updates the plugin through its marketplace, so say nothing there.
...
1. **Detect**, following `.opencode/archflow/skills/archflow/stack-detection.md`. That file is the
...
On OpenCode the plugin's `PreToolUse` hook stops an agent force-pushing to `main` or pushing during
...
```bash
sh ".opencode/archflow/scripts/archflow-install-git-guard.sh"
```
