...
Configure external tool MCP servers for use with OpenCode.
...
## Registry
The curated MCP registry is stored in `.opencode/archflow/skills/archflow/mcp-registry.yaml`. Read that file to look up tool details. Edit it to add or update MCP servers.
...
Read `.opencode/archflow/skills/archflow/mcp-registry.yaml` and search for the tool name (case-insensitive). If found, show:
...
> "Configured [Tool] MCP. Restart OpenCode or run `/mcp` to verify the connection."
...
When invoked without arguments (`/archflow-setup-mcp`), read `.opencode/archflow/skills/archflow/mcp-registry.yaml` and show the registry as a table:
...
- Some MCPs require OpenCode restart after configuration.
