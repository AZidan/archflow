...
Arguments (`the text the user wrote after the command`): `--validate` also validates every `.archflow/` state file against its
...
| `claude` on PATH | **WARN** | Only `/archflow-studio` needs it | Already present if Cursor runs |
...
```bash
python3 ".cursor/archflow/scripts/validate_archflow.py" .
```
...
```bash
python3 ".cursor/archflow/scripts/upgrade_archflow.py" . \
  --plugin-root ".cursor/archflow"
```
...
```bash
python3 ".cursor/archflow/scripts/upgrade_archflow.py" . \
  --plugin-root ".cursor/archflow" --apply
```
...
what is installed, the marketplace plugin on Cursor or the copied adapter elsewhere. It reads `~/.cache/archflow-install/latest.json`,
refreshed in the background at most once a day, and is silenced by `update_check: false` in
`project-settings.yaml`. Report the same comparison here when that file exists. The fix on Cursor
...
1. **Detect**, following `.cursor/archflow/skills/archflow/stack-detection.md`. That file is the
...
On Cursor the plugin's `PreToolUse` hook stops an agent force-pushing to `main` or pushing during
...
```bash
sh ".cursor/archflow/scripts/archflow-install-git-guard.sh"
```
