...
Argument (`the text the user wrote after the command`): optional tool name (e.g. `jira`, `notion`). Empty → list the registry.

Configure external tool MCP servers for use with Cursor.
...
## Registry
The curated MCP registry is stored in `.cursor/archflow/skills/archflow/mcp-registry.yaml`. Read that file to look up tool details. Edit it to add or update MCP servers.
...
Read `.cursor/archflow/skills/archflow/mcp-registry.yaml` and search for the tool name (case-insensitive). If found, show:
...
> "Configured [Tool] MCP. Restart Cursor or run `/mcp` to verify the connection."
...
When invoked without arguments (`/archflow-setup-mcp`), read `.cursor/archflow/skills/archflow/mcp-registry.yaml` and show the registry as a table:
...
- Some MCPs require Cursor restart after configuration.
