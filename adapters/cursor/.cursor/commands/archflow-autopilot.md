...
Argument (`the text the user wrote after the command`): optional. A list of story ids to run, or one of the subcommands below.
...
`.cursor/archflow/commands/groom.md` inline — the user is still here) / `Drop it from this run`.
...
building on an unknown base. Re-ask (via `a direct question to the user (wait for the reply before continuing)`) only questions for stories that were
