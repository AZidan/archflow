...
Match `the text the user wrote after the command` against the story-id pattern from `release-schema.yaml`:
...
- Design-system files live in `.archflow/design-systems/`, copied there by `/archflow-init` and
  `/archflow-onboard` from `.cursor/archflow/skills/archflow/design-systems/`. If the
