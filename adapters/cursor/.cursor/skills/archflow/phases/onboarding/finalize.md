...
2. **Copy workflow.md, phases, schemas and design-systems into the project's `.archflow/`:**
   - Copy `.cursor/archflow/skills/archflow/workflow.md` → `.archflow/workflow.md`
   - Copy `.cursor/archflow/skills/archflow/phases/` → `.archflow/phases/` (skip files that already exist)
   - Copy `.cursor/archflow/skills/archflow/schemas/` → `.archflow/schemas/` (skip files that already exist)
   - Copy `.cursor/archflow/skills/archflow/design-systems/` → `.archflow/design-systems/` (skip files that already exist; skip the whole directory for `backend_only`)
   - Copy `.cursor/archflow/skills/archflow/stacks/` → `.archflow/stacks/` (skip files that already exist)
   - Copy `.cursor/archflow/skills/archflow/test-accounts.example.yaml` → `.archflow/` (skip if present)
...
This file provides guidance to Cursor (claude.ai/code) when working with code in this repository.
