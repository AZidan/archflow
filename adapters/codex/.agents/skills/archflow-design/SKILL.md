...
> Invoke with `$archflow-design`. Arguments are the text after the mention.

Arguments: `[pick|list|<system-name>|<story-id>]`
...
Match `the text the user wrote after the skill mention` against the story-id pattern from `release-schema.yaml`:
...
- Design-system files live in `.archflow/design-systems/`, copied there by `$archflow-init` and
  `$archflow-onboard` from `.codex/archflow/skills/archflow/design-systems/`. If the
