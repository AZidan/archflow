...
> Invoke with `$archflow-doctor`. Arguments are the text after the mention.

Arguments: `[--validate] [--fix]`


# $archflow-doctor — Environment and project health check

Arguments (`the text the user wrote after the skill mention`): `--validate` also validates every `.archflow/` state file against its
...
| `claude` on PATH | **WARN** | Only `$archflow-studio` needs it | Already present if Codex runs |
...
```bash
python3 ".codex/archflow/scripts/validate_archflow.py" .
```
...
```bash
python3 ".codex/archflow/scripts/upgrade_archflow.py" . \
  --plugin-root ".codex/archflow"
```
...
```bash
python3 ".codex/archflow/scripts/upgrade_archflow.py" . \
  --plugin-root ".codex/archflow" --apply
```
...
what is installed, the marketplace plugin on Codex or the copied adapter elsewhere. It reads `~/.cache/archflow-install/latest.json`,
refreshed in the background at most once a day, and is silenced by `update_check: false` in
`project-settings.yaml`. Report the same comparison here when that file exists. The fix on Codex
...
1. **Detect**, following `.codex/archflow/skills/archflow/stack-detection.md`. That file is the
...
On Codex the plugin's `PreToolUse` hook stops an agent force-pushing to `main` or pushing during
...
```bash
sh ".codex/archflow/scripts/archflow-install-git-guard.sh"
```
