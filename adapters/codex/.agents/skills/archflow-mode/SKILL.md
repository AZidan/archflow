...
> Invoke with `$archflow-mode`. Arguments are the text after the mention.
...
Argument (`the text the user wrote after the skill mention`): optional target mode `quick` or `full`. Empty → show the current mode.
