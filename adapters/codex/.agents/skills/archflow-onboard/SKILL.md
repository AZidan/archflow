...
> Invoke with `$archflow-onboard`. Arguments are the text after the mention.
...
- **Yes** — copy each missing file from `.codex/archflow/skills/archflow/` to `.archflow/`. Create subdirectories (`phases/`, `schemas/`, `design-systems/`, `design-systems/examples/`) if needed. NEVER overwrite existing files.
...
1. **Detect the stack.** Read and follow
   `.codex/archflow/skills/archflow/stack-detection.md` — the evidence sources, the
...
2. Tell the user: "Please restart Codex, then run `$archflow-onboard` to resume."
