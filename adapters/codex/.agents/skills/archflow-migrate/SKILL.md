...
> Invoke with `$archflow-migrate`. Arguments are the text after the mention.
...
The migration is performed by **`.codex/archflow/scripts/migrate.py`**, which
...
```bash
python3 .codex/archflow/scripts/migrate.py --path <project-root> --dry-run
```
...
```bash
python3 .codex/archflow/scripts/migrate.py --path <project-root> --apply --active <sprint-id>
```
