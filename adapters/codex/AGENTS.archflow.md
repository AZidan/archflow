...
**At the start of every session, read `.archflow/instructions.md` before doing anything else.** It is the always-loaded core; `.archflow/reference.md` holds detail to read on demand.
...
Specialised agents live in `.codex/agents/` and are dispatched with `spawn_agent`. Phase 3 runs `ui-engineer` and `api-engineer` in parallel against the same API contract; if multi-agent is unavailable, run them serially in that order — the contract is what keeps them consistent, not the concurrency.
