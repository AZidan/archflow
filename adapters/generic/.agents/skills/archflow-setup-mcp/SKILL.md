---
name: archflow-setup-mcp
description: "Use ONLY when the user asks for $archflow-setup-mcp or \"archflow setup-mcp\". Connect an external tool via MCP (Jira, Notion, Linear, GitHub, SuperDesign, ...)"
---
...
Argument (`the text the user wrote after the skill name`): optional tool name (e.g. `jira`, `notion`). Empty → list the registry.

Configure external tool MCP servers for use with your coding agent.
...
## Registry
The curated MCP registry is stored in `.agents/archflow/skills/archflow/mcp-registry.yaml`. Read that file to look up tool details. Edit it to add or update MCP servers.
...
Read `.agents/archflow/skills/archflow/mcp-registry.yaml` and search for the tool name (case-insensitive). If found, show:
...
> "Configured [Tool] MCP. Restart your coding agent or run `/mcp` to verify the connection."
...
When invoked without arguments (`$archflow-setup-mcp`), read `.agents/archflow/skills/archflow/mcp-registry.yaml` and show the registry as a table:
...
- Some MCPs require your coding agent restart after configuration.
