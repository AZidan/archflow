---
name: archflow-migrate
description: "Use ONLY when the user asks for $archflow-migrate or \"archflow migrate\". Upgrade a v1.0 archflow-onboarded project (sprints) to schema v2.0 (releases) — dry-run first, then apply"
---
...
The migration is performed by **`.agents/archflow/scripts/migrate.py`**, which
...
```bash
python3 .agents/archflow/scripts/migrate.py --path <project-root> --dry-run
```
...
```bash
python3 .agents/archflow/scripts/migrate.py --path <project-root> --apply --active <sprint-id>
```
