...
2. **Copy workflow.md, phases, schemas and design-systems into the project's `.archflow/`:**
   - Copy `.agents/archflow/skills/archflow/workflow.md` → `.archflow/workflow.md`
   - Copy `.agents/archflow/skills/archflow/phases/` → `.archflow/phases/` (skip files that already exist)
   - Copy `.agents/archflow/skills/archflow/schemas/` → `.archflow/schemas/` (skip files that already exist)
   - Copy `.agents/archflow/skills/archflow/design-systems/` → `.archflow/design-systems/` (skip files that already exist; skip the whole directory for `backend_only`)
   - Copy `.agents/archflow/skills/archflow/stacks/` → `.archflow/stacks/` (skip files that already exist)
   - Copy `.agents/archflow/skills/archflow/test-accounts.example.yaml` → `.archflow/` (skip if present)
...
This file provides guidance to your coding agent (claude.ai/code) when working with code in this repository.
