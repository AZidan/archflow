---
name: archflow-doctor
description: "Use ONLY when the user asks for $archflow-doctor or \"archflow doctor\". Check the environment and project state — what Archflow needs, what is missing, and how to fix it"
---
...
Arguments (`the text the user wrote after the skill name`): `--validate` also validates every `.archflow/` state file against its
...
| `claude` on PATH | **WARN** | Only `$archflow-studio` needs it | Already present if your coding agent runs |
...
```bash
python3 ".agents/archflow/scripts/validate_archflow.py" .
```
...
```bash
python3 ".agents/archflow/scripts/upgrade_archflow.py" . \
  --plugin-root ".agents/archflow"
```
...
```bash
python3 ".agents/archflow/scripts/upgrade_archflow.py" . \
  --plugin-root ".agents/archflow" --apply
```
...
what is installed, the marketplace plugin on your coding agent or the copied adapter elsewhere. It reads `~/.cache/archflow-install/latest.json`,
refreshed in the background at most once a day, and is silenced by `update_check: false` in
`project-settings.yaml`. Report the same comparison here when that file exists. The fix on your coding agent
...
1. **Detect**, following `.agents/archflow/skills/archflow/stack-detection.md`. That file is the
...
On your coding agent the plugin's `PreToolUse` hook stops an agent force-pushing to `main` or pushing during
...
```bash
sh ".agents/archflow/scripts/archflow-install-git-guard.sh"
```
