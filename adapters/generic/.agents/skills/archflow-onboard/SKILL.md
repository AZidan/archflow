...
description: "Use ONLY when the user asks for $archflow-onboard or \"archflow onboard\". Set up Archflow in an EXISTING codebase — audit code, import context, pick the phase"
---
...
- **Yes** — copy each missing file from `.agents/archflow/skills/archflow/` to `.archflow/`. Create subdirectories (`phases/`, `schemas/`, `design-systems/`, `design-systems/examples/`) if needed. NEVER overwrite existing files.
...
```
- **Yes** — read `.agents/archflow/commands/design.md` and follow **Step 3 — `pick`** inline.
...
1. **Detect the stack.** Read and follow
   `.agents/archflow/skills/archflow/stack-detection.md` — the evidence sources, the
...
1. Check if the tool's MCP is configured:
   ...
   ```
2. If NOT configured:
   - Run `$archflow-setup-mcp [tool]` inline (load `.agents/archflow/commands/setup-mcp.md`)
   - If MCP setup requires restart: save progress to `.onboard-progress.yaml`, instruct user to restart your coding agent, then run `$archflow-onboard` again
...
- **Pick another** (and whenever nothing was detected) — read
  `.agents/archflow/commands/design.md` and follow **Step 3 — `pick`** inline. It filters the
...
**1b. Doc Deep-Dive (Task subagent, (serially, one role at a time))**
...
**1c. Design Extraction (Task subagent, (serially, one role at a time))**
...
**1d. Route/API Extraction (Task subagent, (serially, one role at a time))**
...
If an MCP needs to be configured and requires a your coding agent restart:
1. Save all progress to `.onboard-progress.yaml`
2. Tell the user: "Please restart your coding agent, then run `$archflow-onboard` to resume."
